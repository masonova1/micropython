...
static void fifoWait(const int level)
{
  while (((tft_pio->flevel >> (pio_sm * 8)) & 0x000F) > (8-level)){}
}

static void writeCommand(const uint8_t c)
{
  CS_L; clearDC();
  write8(c);
  setDC(); CS_H;
...
static void writeData(const uint8_t d)
{
  CS_L; setDC();
  write8(d);
  CS_L; CS_H;
...
static void pushBlock16(const uint16_t color, const uint32_t len)
{
  if (len > 0) {
    stallWait();
...
static void pushBlock8(const uint8_t color, const uint32_t len)
{
  if(len & 1)
  {
    stallWait();
...
const static uint16_t tft_io_program_instructions[] = {
...
static pio_sm_config tft_io_program_get_default_config(uint offset) {
