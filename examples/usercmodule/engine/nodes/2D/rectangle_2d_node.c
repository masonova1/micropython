...
STATIC mp_obj_t rectangle_2d_node_class_draw(mp_obj_t self_in, mp_obj_t camera_obj){
    ...
    //         engine_draw_pixel(color, (int32_t)mp_obj_get_float(position->x)+x, (int32_t)mp_obj_get_float(position->y)+y, camera);
    //     }
    // }
    engine_fill_color(color, (int32_t)mp_obj_get_float(position->x), (int32_t)mp_obj_get_float(position->y), width, height, camera);
